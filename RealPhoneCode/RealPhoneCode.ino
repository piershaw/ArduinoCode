...
String myPhone = "13105314587"; 
String myAPN = "wap.tracfone";
...
void SendTextMessage()
{
  ...
  delay(100);
  mySerial.println("AT + CMGS = \"+"+myPhone+"\"");//send sms message, be careful need to add a country code before the cellphone number
  delay(100);
  mySerial.println("Hi starr");//the content of the message
  delay(100);
  ...
  Tft.fillScreen();
  Tft.drawString("Send Text Message", 20, 100, 3, WHITE); 
}

void receiveTextMessage() {
  //if(!BUSY)
  mySerial.print("AT+CMGR=2\r");
   delay(100);
   //ShowSerialData(); 
  //mySerial.println();
  //Tft.fillScreen();
  //Tft.drawString("reading msg...", 20, 100, 3, WHITE); 
}
...
void DialVoiceCall()
{
  mySerial.println("ATD +"+myPhone+"+;");//dial the number
  delay(100);
  mySerial.println();
  Tft.fillScreen();
  Tft.drawString("Dial Voice Call", 20, 100, 3, WHITE); 
}
...
void SubmitHttpRequest()
{
  ...
  mySerial.println("AT+SAPBR=3,1,\"APN\",\""+myAPN+"\"");//setting the APN, the second need you fill in your local apn server
  delay(4000);
