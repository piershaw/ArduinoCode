...
uint8_t UsbHost_::getUsbTaskState( void )
{
    return( usb_task_state );
}

void UsbHost_::setUsbTaskState( uint8_t state )
{
    usb_task_state = state;
}     

EpInfo* UsbHost_::getEpInfoEntry( uint8_t addr, uint8_t ep )
{
...
uint8_t UsbHost_::setEpInfoEntry( uint8_t addr, uint8_t epcount, EpInfo* eprecord_ptr )
{
	if (!eprecord_ptr)
...
uint8_t UsbHost_::ctrlReq( uint8_t addr, uint8_t ep, uint8_t bmReqType, uint8_t bRequest, uint8_t wValLo, uint8_t wValHi, 
...
uint8_t UsbHost_::outTransfer( uint8_t addr, uint8_t ep, uint16_t nbytes, uint8_t* data )
{
	EpInfo		*pep = NULL;
	uint16_t	nak_limit;
...
uint8_t UsbHost_::OutTransfer(EpInfo *pep, uint16_t nak_limit, uint16_t nbytes, uint8_t *data)
{
...
void UsbHost_::Task( void )      //USB state machine
{
	uint8_t rcode;
	uint8_t tmpdata;
...
uint8_t UsbHost_::DefaultAddressing(uint8_t parent, uint8_t port, bool lowspeed)
{
	uint8_t		buf[12];
	uint8_t		rcode;
...
uint8_t UsbHost_::ReleaseDevice(uint8_t addr)
{
	if (!addr)
		return 0;
...
uint8_t UsbHost_::getDevDescr( uint8_t addr, uint8_t ep, uint16_t nbytes, uint8_t* dataptr ) 
{
    return( ctrlReq( addr, ep, bmREQ_GET_DESCR, USB_REQUEST_GET_DESCRIPTOR, 0x00, USB_DESCRIPTOR_DEVICE, 0x0000, nbytes, nbytes, dataptr, NULL ));
}
//get configuration descriptor  
uint8_t UsbHost_::getConfDescr( uint8_t addr, uint8_t ep, uint16_t nbytes, uint8_t conf, uint8_t* dataptr ) 
{
...
uint8_t UsbHost_::getStrDescr( uint8_t addr, uint8_t ep, uint16_t ns, uint8_t index, uint16_t langid, uint8_t* dataptr ) 
{
...
//set configuration
uint8_t UsbHost_::setConf( uint8_t addr, uint8_t ep, uint8_t conf_value ) 
{
